...
static std::string drop_nth_line(const std::vector<std::string> &lines,
                                 size_t nth) {
...
static std::vector<Case> make_cases() {
  ...
  for (int i = 1; i <= 50; ++i) {
    std::vector<std::string> lines = {
        "keep-" + tag2(i) + "-1", "keep-" + tag2(i) + "-2",
        "keep-" + tag2(i) + "-3", "keep-" + tag2(i) + "-4",
        "keep-" + tag2(i) + "-5",
    };
  ...
  for (int i = 1; i <= 50; ++i) {
    std::vector<std::string> lines = {
        ...
        "cab" + tag2(i) + "abc", "aaabbbccc" + tag2(i), "z" + tag2(i) + "cab"};
    std::string input = join_lines(lines);
    cases.push_back({
        "Translit" + tag2(i),
        "y/abc/ABC/",
  ...
  for (int i = 1; i <= 50; ++i) {
    std::vector<std::string> lines = {
        "row-" + tag2(i) + "-1",     "row-" + tag2(i) + "-2x",
        "row-" + tag2(i) + "-3xx",   "row-" + tag2(i) + "-4xxx",
        "row-" + tag2(i) + "-5xxxx", "row-" + tag2(i) + "-6xxxxx",
    };
    cases.push_back({
        "LineNo" + tag2(i),
        "=",
...
TEST_P(IntegrationGenerated, RunsCorrectly) {
  ...
  EXPECT_EQ(run_sed(c.script, c.input, c.suppress, c.extended, c.null_delim),
            c.expected)
      << c.name;
}
