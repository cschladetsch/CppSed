...
struct Pipe {
    int r = -1, w = -1;
    Pipe()  { int fd[2]; if (pipe(fd)) throw std::runtime_error("pipe"); r=fd[0]; w=fd[1]; }
    ...
    std::string drain() {
        ...
        while ((n = read(r, buf, sizeof buf)) > 0) out.append(buf, static_cast<size_t>(n));
        return out;
    }
};

// ── Core helper ───────────────────────────────────────────────
inline std::string run_sed(const std::string& script,
    ...
    if (tmpfd < 0) throw std::runtime_error("mkstemp failed");
    if (!input.empty())
        (void)write(tmpfd, input.data(), input.size());
    close(tmpfd);

    struct Cleanup { const char* p; ~Cleanup(){ unlink(p); } } cleanup{tmppath};
    ...
    // Compile
    Parser p; p.src = script; p.ext = extended;
