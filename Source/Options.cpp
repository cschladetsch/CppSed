...
Options parse_args(int argc, char** argv) {
    ...
    visible.add_options()
        ("expression,e",  po::value<vector<string>>()->composing(), "script")
        ...
        ("regexp-extended,E", po::bool_switch(&opts.extended_re),   "extended regex")
        ("separate,s",    po::bool_switch(&opts.separate),          "treat files separately")
        ("null-data,z",   po::bool_switch(&opts.null_delim),        "NUL-delimited lines")
        ("sandbox",       po::bool_switch(&opts.sandbox),           "disable e/r/w commands")
        ("help",          "show this help")
    ;
    ...
    hidden.add_options()
        ("regexp,r",    po::bool_switch(&opts.extended_re))
        ("inplace,i",   po::value<string>()->implicit_value(""),    "-i[SUFFIX]")
    ...
    if (vm.count("help")) {
        ...
        std::println(stdout, "{}", [&]{ std::ostringstream os; os << visible; return os.str(); }());
        std::exit(0);
    }
    ...
    if (vm.count("file")) {
        for (const string& path : vm["file"].as<vector<string>>()) {
            ...
            if (!ifs) die(std::format("cannot open script file '{}'", path));
    ...
    if (combined.empty()) {
        const auto* pos_files =
            vm.count("input-file") ? &vm["input-file"].as<vector<string>>() : nullptr;
