...
string Parser::parse_re_body(char d) {
  ...
  if (at_end())
    die(std::format("unterminated regex (expected '{}')", d));
...
void Parser::parse_s(Cmd &cmd) {
  ...
  if (at_end())
    die("s: unterminated replacement");
  get();
  ...
  while (!at_end()) {
    ...
    if (f == 'g') {
      ...
      get();
      cmd.s.wfile = parse_filename();
      break;
    } else if (f == 'm' || f == 'M') {
      get(); /* multi-line (partial) */
    } else
      break;
  }
...
Cmd Parser::parse_one() {
  ...
  if (a0.k != AK::None) {
    ...
    if (eat(',')) {
      ...
      Addr a1 = parse_addr();
      if (a1.k == AK::None)
        die("expected second address after ','");
      cmd.a[1] = a1;
      cmd.naddr = 2;
    }
  }
  ...
  case 'l':
    ...
    break;
  case '{':
    cmd.block = parse_cmds(/*in_block=*/true);
    break;
  case 'e':
    if (!at_end() && peek() != '\n' && peek() != ';' && peek() != '}')
      cmd.text = parse_filename();
