...
void LineSource::add_stdin() {
    Seg seg; seg.fname = "(stdin)"; seg.is_pipe = true;
    segs_.push_back(std::move(seg));
}

void LineSource::add_file(const string& path) {
    if (path == "-") { add_stdin(); return; }

    Seg seg; seg.fname = path;
    ...
    while (si_ < static_cast<int>(segs_.size())) {
        ...
        if (seg.is_pipe) {
            ...
            while ((c = fgetc(stdin)) != EOF && c != static_cast<int>(delim_))
                { line += static_cast<char>(c); got = true; }
            if (c == static_cast<int>(delim_)) got = true;
            if (got) { fname = seg.fname; return true; }
            ++si_; continue;
        }
...
bool LineSource::prime() {
    if (have_peek_) return !peeked_eof_;
    have_peek_  = true;
...
bool LineSource::read(string& line, string& fname, bool& is_last) {
    if (!prime() || peeked_eof_) return false;
