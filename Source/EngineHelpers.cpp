...
void do_trans(const string ytbl[2], string &ps) {
  ...
  for (size_t i = 0; i < s.size() && i < d.size(); ++i)
    tbl[static_cast<unsigned char>(s[i])] = static_cast<unsigned char>(d[i]);
  for (char &c : ps)
...
static bool do_subst_prefix(const SubstOpt &s, string &ps) {
  string_view lit;
  if (!s.re || s.re->src != "^" || s.global || s.nth > 1 ||
      !repl_is_plain_literal(s.repl, lit))
    return false;
  ps.insert(0, lit);
  return true;
}
...
static bool do_subst_literal(const RE &re, const SubstOpt &s, string &ps) {
  if (!re.is_literal() || s.icase)
    return false;

  string_view repl_lit;
  if (!repl_is_plain_literal(s.repl, repl_lit))
    return false;
  ...
  while (cur < end) {
    ...
    ++mnum;
    bool do_it = s.global ? (s.nth == 0 || mnum >= s.nth) : (mnum == nth);
...
bool do_subst(const SubstOpt &s, string &ps, const shared_ptr<RE> &last_re) {
  ...
  if (!re || !re->ok)
    die("no previous regex");

  if (do_subst_prefix(s, ps))
    return true;
  if (do_subst_literal(*re, s, ps))
    return true;
