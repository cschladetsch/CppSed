...
void Linker::flatten(const CmdVec& cmds) {
    for (const Cmd& c : cmds) {
        FlatCmd fc;
        fc.op     = c.op;     fc.naddr  = c.naddr;  fc.neg    = c.neg;
        fc.a[0]   = c.a[0];  fc.a[1]   = c.a[1];
        fc.text   = c.text;  fc.s      = c.s;
        fc.ytbl[0]= c.ytbl[0]; fc.ytbl[1]= c.ytbl[1];
        fc.ecode  = c.ecode; fc.lwidth = c.lwidth;
        fc.id     = next_id++;
        ...
        if (c.op == ':') {
            ...
            prog.push_back(fc);
            flatten(c.block);
            FlatCmd sentinel; sentinel.op = '}'; sentinel.id = next_id++;
