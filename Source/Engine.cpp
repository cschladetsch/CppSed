...
bool Engine::matches(const FlatCmd &c) {
  ...
  if (c.naddr == 0) {
    ...
    if (entered < 0) {
      ...
      bool m1 = (c.a[0].k == AK::Line0) ? (lineno <= 1) : addr_single(c.a[0]);
      if (m1) {
        entered = lineno;
...
FC Engine::run(LineSource &src) {
  ...
  while (pc < N) {
    ...
    if (c.a[0].k == AK::RE && c.a[0].re && c.a[0].re->ok)
      last_re = c.a[0].re;
    if (c.a[1].k == AK::RE && c.a[1].re && c.a[1].re->ok)
      last_re = c.a[1].re;
    ...
    case 'e':
      if (sandbox)
        die("'e' command disabled in sandbox mode");
      if (c.text.empty())
        ps = exec_shell(ps);
      else
        out->writeln(exec_shell(c.text));
      break;
    ...
    case 'r':
      deferred.push_back({true, false, c.text});
      break;
    case 'R':
      deferred.push_back({true, true, c.text});
      break;
    ...
    case 's':
      if (do_subst(c.s, ps, last_re)) {
        ...
        if (c.s.print)
          out->writeln(ps);
        if (!c.s.wfile.empty())
          write_to_file(c.s.wfile, ps);
        if (c.s.exec) {
          if (sandbox)
            die("s///e disabled in sandbox mode");
          ps = exec_shell(ps);
        }
      }
      break;
    ...
    // ── w / W: write to file ──────────────────────────────
    case 'w':
      write_to_file(c.text, ps);
      break;
    case 'W': {
      auto nl = ps.find('\n');
      write_to_file(c.text, nl == string::npos ? string_view(ps)
                                               : string_view(ps).substr(0, nl));
      break;
    }
