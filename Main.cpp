...
struct InplaceGuard {
    ...
    ~InplaceGuard() {
        if (tmp_fd >= 0) { ::close(tmp_fd); ::unlink(tmp.c_str()); }
    }
...
int main(int argc, char** argv) {
    ...
    if (opts.first_hash_n) opts.suppress = true;
